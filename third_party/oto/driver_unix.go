...
func (c *context) readAndWrite(buf32 []float32) bool {
	...
	for len(buf32) > 0 {
		...
		if n < 0 {
			// PipeWire's ALSA bridge can report a transient I/O error when the
			// desktop is under load. Prepare the PCM and retry the same frames
			// instead of terminating the playback worker.
			if prepareErr := C.snd_pcm_prepare(c.handle); prepareErr < 0 {
